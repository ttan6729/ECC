...
int CommandDistance::run(vector<string> list, vector<vector<double>>& vv) 
{
    ...
    SketchQuery->initFromFiles(list, parameters, 0, true);
    double lengthThreshold = (parameters.warning * sketchRef.getKmerSpace()) / (1. - parameters.warning);
    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = i+1; j < n; j++)
        {
            dist = compareSketches(SketchQuery,SketchQuery,distanceMax,pValueMax, i, j );
            vv[i][j] = dist;
            vv[j][i] = dist;
        }
...
int CommandDistance::run(string file1, string file2, double *dist) 
{
    bool list = options.at("list").active;
    bool table = options.at("table").active;
    //bool log = options.at("log").active;
    ...
    const string & fileReference = file1;
    ...
    refArgVector.push_back(fileReference);
    //cerr << "Sketch for " << fileReference << " not found or out of date; creating..." << endl;
    Sketch *SketchRef = new Sketch();
    SketchRef->initFromFiles(refArgVector, parameters);
    
    double lengthThreshold = (parameters.warning * sketchRef.getKmerSpace()) / (1. - parameters.warning);
//    ThreadPool<CompareInput, CompareOutput> threadPool(compare, threads);
    vector<string> queryFiles;
    queryFiles.push_back(file2.c_str());
    Sketch *SketchQuery = new Sketch();
    
    SketchQuery->initFromFiles(queryFiles, parameters, 0, true);
//    printf("line 292\n"); 
    (*dist) = compareSketches(SketchRef,SketchQuery,distanceMax,pValueMax );
//   printf("line 294\n"); 
    delete SketchRef;
    delete SketchQuery;
    return 0;
...
CommandDistance::CompareOutput * compare(CommandDistance::CompareInput * input)
{
    ...
    {
        compareSketches(&output->pairs[k], sketchRef.getReference(j), sketchQuery.getReference(i), sketchSize, sketchRef.getKmerSize(), 
            sketchRef.getKmerSpace(), input->maxDistance, input->maxPValue);
...
{
    const HashList & hashesSortedRef = sketchRef->getReference(index1).hashesSorted;
    const HashList & hashesSortedQry = sketchQuery->getReference(index2).hashesSorted;
    uint64_t sketchSize = sketchQuery->getMinHashesPerWindow() < sketchRef->getMinHashesPerWindow() ?
             sketchQuery->getMinHashesPerWindow() :
             sketchRef->getMinHashesPerWindow();
    int kmerSize = sketchRef->getKmerSize();
    int kmerSpace = sketchRef->getKmerSpace();
...
{
    const HashList & hashesSortedRef = sketchRef->getReference(0).hashesSorted;
    const HashList & hashesSortedQry = sketchQuery->getReference(0).hashesSorted;
    uint64_t sketchSize = sketchQuery->getMinHashesPerWindow() < sketchRef->getMinHashesPerWindow() ?
             sketchQuery->getMinHashesPerWindow() :
             sketchRef->getMinHashesPerWindow();
    int kmerSize = sketchRef->getKmerSize();
