...
int Sketch::initFromFiles(const vector<string> & files, const Parameters & parametersNew, int verbosity, bool enforceParameters, bool contain)
{
    ...
    for ( int i = 0; i < files.size(); i++ )
    {
        ...
        if ( isSketch )
        {
			...
			//
			Sketch sketchTest;
			sketchTest.initParametersFromCapnp(files[i].c_str(),0);
         	if ( i == 0 && ! enforceParameters )
        	{
        		initParametersFromCapnp(files[i].c_str(),0);
        	}
...
//same as initparametersFromCapnp, the only difference is that the maximum legnth of each file is 10e5
uint64_t Sketch::initParametersFromCapnp(const char * file, bool length_limited)
{
	...
	uint64_t file_size = fileInfo.st_size > 100000 ? 100000 : fileInfo.st_size;
    ...
    capnp::FlatArrayMessageReader * message = new capnp::FlatArrayMessageReader(kj::ArrayPtr<const capnp::word>(reinterpret_cast<const capnp::word *>(data), file_size / sizeof(capnp::word)), readerOptions);
